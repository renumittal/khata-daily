...
-- ---------- Auth hierarchy, groups, and projects ----------
-- Schema-only pass: no login/session UI yet — see the TEMPORARY RLS note
-- at the bottom of this file, which applies unchanged to these tables too.
-- Defined ahead of `transactions` since it references `project`/`app_user`.
...
create table if not exists transactions (
  ...
  -- Mirrors the `person` column's style (a human-readable FK, not a
  -- surrogate id) rather than project_id, so the ledger stays as readable
  -- as it already is. Both null for ordinary non-project khata entries.
  project text references project(name),
  paid_by_user bigint references app_user(id)
);

-- Junction tables below carry a validity window (valid_from/valid_to)
-- instead of a plain composite primary key: ending a membership/grant sets
-- valid_to rather than deleting the row, so history isn't lost when a
-- person changes group or a level's access is revoked. A partial unique
-- index enforces at most one currently-active row per pair.
...
-- ---------- Auth hierarchy / groups / project helpers ----------
